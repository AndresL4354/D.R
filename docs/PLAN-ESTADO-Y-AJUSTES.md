...
| **3. Dominios core** | Despacho(+trigger), Proyecto(asociar/oficializar), Cuadrilla, EntregaEPP, Mochila SPDC | 🟡 **~30%** | Vistas list/detail de Despacho, Proyecto, EntregaEPP, Mochila SPDC ✅ (lectura). Trigger auditoría despacho (`fn_audita_estado_despacho`) ✅. **Falta:** todas las **mutaciones de negocio** (asociar personal, oficializar, máquina de estados con confirmación/permiso/auditoría en UI), y **Cuadrilla** (no hay feature). |
...
2. **Fase 2.5 — Paridad visual (transversal):** clonar **PersonaList** completo (card Filtros + columnas reales + kebab + "Cargue Personas"), establecer el patrón de listado y replicarlo en los demás list/detail ya existentes. Captura lado-a-lado como gate.
